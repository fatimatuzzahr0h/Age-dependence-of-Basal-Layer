"""
Tahap 3.3 - Batch production 18 skenario (6 set x 3 standoff)
Resep final:
- N = 1e7 histori per skenario (target unc basal <= 2%)
- DoseActor 1x1x1 per lapisan (volume-restricted)
- Aktor silinder profil kedalaman (250 bin x 0.1 mm)
- Konversi dosis: D[Gy/(Bq.s)] = 2 * D[Gy/hist]
- Resume capability: cek file marker <tag>_s<standoff>.done
- Paralel: jalankan beberapa instans dengan argumen berbeda
Output: D:\\Brachy\\production_output\\<tag>\\s<standoff>\\*.mhd
Ringkasan: D:\\Brachy\\production_output\\production_summary.json
CLI:
  python 22_batch_production.py                    # semua 18 skenario
  python 22_batch_production.py MRCP_01M MRCP_01F  # subset (paralel)
"""
import sys
import json
import time
...
OUT = Path(r"D:\Brachy\production_output")

ALL_TAGS = ["MRCP_01M", "MRCP_01F", "MRCP_05M", "MRCP_05F", "MRCP_15M", "MRCP_15F"]
...
def run_scenario(tag, s, meds_sel, off, geo, a, n, h, R, out_dir, seed):
    sim = gate.Simulation()
...
def main():
    tags = sys.argv[1:] if len(sys.argv) > 1 else ALL_TAGS
    OUT.mkdir(exist_ok=True)
    print("=" * 78)
    print(f"TAHAP 3.3 - BATCH PRODUCTION | {len(tags)} set x 3 standoff | N={N_HIST:.0e}")
    print("=" * 78)
    ...
    for tag in tags:
        ...
        for s in STANDOFFS:
            out_dir = OUT / tag / f"s{s:.1f}"
            marker = out_dir / "done.json"
            if marker.exists():
                print(f"[SKIP] {tag} s={s:.1f} sudah selesai")
            ...
            print(f"\n--- {tag} s={s:.1f} mm | seed={seed} ---")
            dt = run_scenario(tag, s, meds_sel, off, geo, a, n, h, R, out_dir, seed)
            print(f"run selesai {dt:.1f} s ({dt/60:.1f} min)")

            row = dict(tag=tag, standoff_mm=s, n_hist=N_HIST, seed=seed, layers={}, profile={})
            for L in LAYERS:
                dose_arr = read_image(out_dir / f"dose_{L}.mhd")
                unc_arr = read_image(out_dir / f"unc_{L}.mhd")
            ...
            cyl_dose = read_image(out_dir / "cyl_dose.mhd").flatten()
            cyl_unc = read_image(out_dir / "cyl_unc.mhd").flatten()
    ...
    out_json = OUT / "production_summary.json"
    with open(out_json, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nRingkasan final: {out_json}")
    print(f"Total skenario selesai: {len(all_results)}/{len(tags) * len(STANDOFFS)}")
    print("=" * 78)
    print("TAHAP 3.3 SELESAI" if len(all_results) == len(tags) * len(STANDOFFS) else "TAHAP 3.3: sebagian skenario belum selesai")
    print("=" * 78)
